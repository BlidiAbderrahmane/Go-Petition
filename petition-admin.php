...
if (isset($_SESSION['mailadmin'])) {
	include "db_conn.php";
	$id=$_GET['id'];
	$query1="select p.titre, p.texte, p.date, m.nom, m.prenom from petition p, membre m where p.mail=m.mail and p.id=$id";
	...
	$query2="select m.nom, m.prenom, r.commentaire from participation r, membre m, petition p where r.mail=m.mail and r.id=p.id and r.id=$id and vote='pour'";
	$result2=mysqli_query($conn,$query2);
	$dataRow2="";
	while ($row2=mysqli_fetch_array($result2)) {
		$dataRow2= $dataRow2."<tr><td>".$row2[0]."</td><td>".$row2[1]."</td><td>".$row2[2]."</td></tr>";
	}
	$query3="select m.nom, m.prenom, r.commentaire from participation r, membre m, petition p where r.mail=m.mail and r.id=p.id and r.id=$id and vote='contre'";
...
<title>Editer une pétition - Admin</title>
<!-- Bootstrap -->
  ...
  <div class="row">
    <div class="border-right col-md-12">
	  ...
	  <?php if (isset($_GET['error'])) { ?> 
		...
		<div class="mt-5 text-center">
		<button onclick="document.getElementById('id01').style.display='block'" class="btn btn-danger">Supprimer la pétition</button>
		<div id="id01" class="modal">
		<span onclick="document.getElementById('id01').style.display='none'" class="close" title="Close Modal">&times;</span>
		<form class="modal-content" method="post" action="supprimer.php">
			<div class="container">
			<h1>Suppression du Pétition N°<?php echo $id;?></h1>
			<p style="font-size: 20px">Êtes vous sur de supprimer cette pétition ?</p>
			<div class="clearfix">
	...
	<div class="row">
		<div class="col-6">
			<h5 class="text-center" style="margin-bottom: 20px">Utilisateurs POUR</h5>
		...
		<div class="col-6">
			<h5 class="text-center" style="margin-bottom: 20px">Utilisateurs CONTRE</h5>
			<table class="table table-bordered table-striped table-hover">
				<thead class="thead-dark">
					<tr>
						<th>Nom</th>
